 ...
 */
stock MC_PrintToChat(int client, const char[] message, any:...)
{
	MC_CheckTrie();
 ...
 * @noreturn
 */
stock MC_PrintToChatAll(const char[] message, any:...)
{
	MC_CheckTrie();
	char buffer[MAX_BUFFER_LENGTH];
	char buffer2[MAX_BUFFER_LENGTH];
 ...
 */
stock MC_PrintToChatEx(int client, int author, const char[] message, any:...)
{
	MC_CheckTrie();
	if (client <= 0 || client > MaxClients)
	{
 ...
 */
stock MC_PrintToChatAllEx(int author, const char[] message, any:...)
{
	MC_CheckTrie();
	if (author <= 0 || author > MaxClients)
	{
 ...
 * @noreturn
 */
stock MC_SendMessage(int client, const char[] message, int author=0)
{
	if (author == 0)
	{
		author = client;
 ...
 * @noreturn
 */
stock MC_SkipNextClient(int client)
{
	if (client <= 0 || client > MaxClients)
	{
		ThrowError("Invalid client index %i", client);
 ...
 */
stock MC_ReplaceColorCodes(char[] buffer, int author=0, bool removeTags=false, int maxlen=MAX_BUFFER_LENGTH)
{
	MC_CheckTrie();
	if (!removeTags)
	{
 ...
 * @noreturn
 */
stock CSubString(const char[] input, char[] output, int maxlen, int start, int numChars=0)
{
	int i = 0;
	for(;;)
	{
 ...
 * @noreturn
 */
stock MC_StrToLower(char[] buffer)
{
 ...
 * @noreturn
 */
stock MC_RemoveTags(char[] message, int maxlen)
{
 ...
 * @noreturn
 */
stock MC_ReplyToCommand(int client, const char[] message, any:...)
{
	char buffer[MAX_BUFFER_LENGTH];
	SetGlobalTransTarget(client);
 ...
 * @noreturn
 */
stock MC_ReplyToCommandEx(int client, int author, const char[] message, any:...)
{
	char buffer[MAX_BUFFER_LENGTH];
	SetGlobalTransTarget(client);
 ...
 * @error
 */
stock MC_ShowActivity(int client, const char[] format, any:...)
{
 ...
 * @error
 */
stock MC_ShowActivityEx(int client, const char[] tag, const char[] format, any:...)
{
	if (sm_show_activity == null)
	{
		sm_show_activity = FindConVar("sm_show_activity");
 ...
 * @error
 */
stock MC_ShowActivity2(int client, const char[] tag, const char[] format, any:...)
{
	if (sm_show_activity == null)
	{
		sm_show_activity = FindConVar("sm_show_activity");
 ...
 */
stock CGetTeamColor(int client)
{
	if (client <= 0 || client > MaxClients)
	{
		ThrowError("Invalid client index %i", client);
